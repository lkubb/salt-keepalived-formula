...
We will see a quick setup to configure the ``keepalived`` formula for different DNS domain names and several roles.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/keepalived/parameters/dns:domain/
    ln -s dns:domain /srv/salt/keepalived/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/keepalived/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/keepalived/parameters/dns:domain/example.com.yaml.jinja``:
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/keepalived/parameters/roles
...
We create a configuration for the role ``keepalived/server`` in ``/srv/salt/keepalived/parameters/roles/keepalived/server.yaml``:
...
We create another configuration for the role ``keepalived/client`` in ``/srv/salt/keepalived/parameters/roles/keepalived/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles keepalived/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles keepalived/client
...
.. code-block:: console
    ...
    mv /srv/salt/keepalived/parameters/dns:domain/  '/srv/salt/keepalived/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-keepalived-as-json:
      test.nop:
        - name: {{ keepalived | json }}
...
Here is an example based on `template-formula/keepalived/config/file.sls`_:
...
This ``sls`` file expose a ``keepalived`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ keepalived['winner'] }}
...
.. _template-formula/keepalived/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/keepalived/config/file.sls
